...
const state = {
  ...
  activePeerId: null, stopMessages: null, stopInbox: null, stopTyping: null, stopClears: null, stopThreadSummaries: {}, signUp: false,
  replyTo: null, pendingImageFile: null, inboxReady: false, messagesLoaded: false, typingTimer: null, typingExpiryTimer: null
};
...
function renderMessages(rawMessages, jumpToLatest = false) {
  ...
  list.innerHTML = rows.map((message) => {
    ...
    return `<div class="message-row${mine ? ' me' : ''}"><div><div class="message-bubble" data-message="${escapeHtml(message.id)}">${quote}${escapeHtml(message.text || '')}${image}</div><div class="message-meta"><div class="message-time">${formatTime(message.timestamp)}</div>${message.editedAt ? '<span class="edited-label">Edited</span>' : ''}</div>${reactionSummary ? `<div class="reaction-summary">${reactionSummary}</div>` : ''}</div></div>`;
  }).join('');
...
function openThread(threadId, peerId) {
  ...
  state.stopMessages = onValue(query(ref(db, `chatMessages/${threadId}`), limitToLast(60)), (snapshot) => { const firstLoad = !state.messagesLoaded; state.messagesLoaded = true; renderMessages(snapshot.val(), firstLoad); });
  watchTyping(threadId); $('message-input').focus();
}
...
function closeActiveChat() {
  ...
  state.stopMessages = null; state.stopTyping = null; state.activeThreadId = null; state.activePeerId = null; state.messages = {}; state.messagesLoaded = false; state.typing = {}; clearReply(); closeMessageMenu();
