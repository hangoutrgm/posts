...
onValue(ref(db, 'settings'), (snap) => {
  if (snap.exists()) {
    ...
    chatSettings.chatGameRounds = s.chatGameRaceTo ?? s.chatGameRounds ?? 5;
    chatSettings.chatGameRaceTo = s.chatGameRaceTo ?? s.chatGameRounds ?? 5;
    chatSettings.chatGameCooldownSec = s.chatGameCooldownSec ?? 60;
    chatSettings.chatGameLbReward = s.chatGameLbReward ?? 0;
    chatSettings.chatGameHostLbReward = s.chatGameHostLbReward ?? 0;
    ...
    const _bm = Number(s.lbBoostMultiplier);
...
function renderMessages(rawMessages, jumpToLatest = false) {
  ...
  const htmlArr = rows.map((message) => {
    ...
    return `<div id="message-${escapeHtml(message.id)}" class="message-row${mine ? ' me' : ''}${isGameCard ? ' is-game-row' : ''}"><div>${senderNameHtml}<div class="message-bubble${isGameCard ? ' is-game-bubble' : ''}${emojiOnly ? ' emoji-only' : ''}"${emojiOnly ? ` style="font-size:${emojiOnlyCount > 1 ? '37px' : '46px'};"` : ''} data-message="${escapeHtml(message.id)}">${noSwipeHint ? '' : '<span class="swipe-reply-hint"><svg viewBox="0 0 24 24"><polyline points="9 17 4 12 9 7"/><path d="M20 18v-2a4 4 0 0 0-4-4H4"/></svg></span>'}${quote}${bumpTag}${messageText}${image}${audioHtml}</div>${!isGameCard && !isGameBump && reactionSummary ? `<div class="reaction-summary">${reactionSummary}</div>` : ''}${metaHtml}</div></div>`;
  }).forEach((html, i) => {
