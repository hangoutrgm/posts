...
window.generatePostHTML = function(post, prefix, filterContext) {
    ...
    if (post.isRepostedGame && post.isRepost) {
        ...
        if (post.gameType === 'first_to_mine') {
        ...
        } else if (post.gameType === 'connect4_pro_max') {
            const isHost = window.currentUser && window.currentUser.uid === post.authorId;
            const playerR = post.proFourPlayerR;
            const playerY = post.proFourPlayerY;
            const playerB = post.proFourPlayerB;
            const playerG = post.proFourPlayerG;
            const nameR = (playerR ? (window.globalUsersCache[playerR]?.name || 'Host') : 'Host');
            const nameY = (playerY ? (window.globalUsersCache[playerY]?.name || 'Player') : 'Open Slot');
            const nameB = (playerB ? (window.globalUsersCache[playerB]?.name || 'Player') : 'Open Slot');
            const nameG = (playerG ? (window.globalUsersCache[playerG]?.name || 'Player') : 'Open Slot');
            const isTargeted = Boolean(post.proFourTargetUser);
            const myUid = window.currentUser ? window.currentUser.uid : null;
            const isAlreadyIn = myUid && (myUid === playerR || myUid === playerY || myUid === playerB || myUid === playerG);
            const isEligibleChallenger = myUid && !isAlreadyIn && (!isTargeted || post.proFourTargetUser === myUid || (playerY && !playerB) || (playerB && !playerG));
            ...
            const isMyTurn = myUid && (
                ...
                (turn === 'Y' && myUid === playerY) ||
                (turn === 'B' && myUid === playerB) ||
                (turn === 'G' && myUid === playerG)
            );

            const turnName = (turn === 'R' ? nameR : (turn === 'Y' ? nameY : (turn === 'B' ? nameB : nameG)));
            const turnColor = turn === 'R' ? 'Red 🔴' : (turn === 'Y' ? 'Yellow 🟡' : (turn === 'B' ? 'Blue 🔵' : 'Green 🟢'));

            const playersBadges = `
                <div class="flex flex-wrap items-center justify-between w-full max-w-sm text-xs font-bold mb-2">
                    <span class="text-rose-600 dark:text-rose-400">🔴 @${escapeHtml(nameR)}</span>
                    <span class="text-amber-600 dark:text-amber-400">🟡 @${escapeHtml(nameY)}</span>
                    <span class="text-blue-600 dark:text-blue-400">🔵 @${escapeHtml(nameB)}</span>
                    <span class="text-green-600 dark:text-green-400">🟢 @${escapeHtml(nameG)}</span>
                </div>`;

            if (post.gameStatus === 'active') {
                if (post.proFourStatus === 'waiting') {
                    const acceptBtn = isEligibleChallenger ? `
                        <button onclick="window.acceptConnect4ProMaxChallenge('${post.id}')" class="mt-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-black py-2.5 px-8 rounded-full shadow-lg transform transition hover:scale-105 active:scale-95 animate-pulse text-sm">
                            <i class="fa-solid fa-circle-arrow-down mr-2"></i>Accept Challenge & Join!
                        </button>` : (isHost ? `<div class="mt-2 text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-slate-700 px-3 py-1 rounded-full">Waiting for 3 more players to join...</div>` : `<div class="mt-2 text-xs text-gray-400">Waiting for players to join...</div>`);

                    const waitingBadges = `
                        <div class="flex flex-wrap items-center justify-center gap-2 my-2 text-xs font-bold">
                            <span class="bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300 px-2.5 py-1 rounded-full">🔴 @${escapeHtml(nameR)} (Host)</span>
                            <span class="text-gray-400 font-extrabold">VS</span>
                            ${playerY ? `<span class="bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300 px-2.5 py-1 rounded-full">🟡 @${escapeHtml(nameY)}</span>` : '<span class="bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400 px-2.5 py-1 rounded-full">🟡 Open Slot</span>'}
                            ${playerB ? `<span class="bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300 px-2.5 py-1 rounded-full">🔵 @${escapeHtml(nameB)}</span>` : '<span class="bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400 px-2.5 py-1 rounded-full">🔵 Open Slot</span>'}
                            ${playerG ? `<span class="bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300 px-2.5 py-1 rounded-full">🟢 @${escapeHtml(nameG)}</span>` : '<span class="bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400 px-2.5 py-1 rounded-full">🟢 Open Slot</span>'}
                        </div>`;

                    gameHtml = `
                        <div class="mt-3 mb-2 p-4 bg-gradient-to-br from-blue-50 to-emerald-50 dark:from-slate-800 dark:to-slate-800 rounded-2xl border-2 border-blue-300 dark:border-blue-900/50 flex flex-col items-center">
                            ${prizeStr}
                            <h4 class="font-black text-blue-900 dark:text-blue-200 text-base mb-1">🟢🔴 Connect 4 Pro Max (7x9)</h4>
                            ${waitingBadges}
                            ${acceptBtn}
                        </div>`;
                } else {
                    ...
                    for (let c = 0; c < 7; c++) {
                        const canDrop = isMyTurn && (board[c] === ''); // top row empty
                        const btnClick = canDrop ? `onclick="window.makeConnect4ProMaxMove('${post.id}', ${c})"` : '';
                        const btnClass = canDrop ? 'bg-blue-500 hover:bg-blue-400 text-white cursor-pointer active:scale-90 hover:scale-105' : 'bg-transparent text-transparent cursor-default';
                        dropBtnsHtml += `<button type="button" ${btnClick} class="w-full py-1 text-xs font-black rounded-lg transition ${btnClass}">⬇</button>`;
                    }
                    dropBtnsHtml += `</div>`;

                    // 7 columns x 9 rows board
                    let gridHtml = `<div class="grid grid-cols-7 grid-rows-9 gap-1 p-2.5 bg-blue-600 dark:bg-blue-800 rounded-2xl shadow-xl border-4 border-blue-700 dark:border-blue-900 w-72 h-96 mx-auto my-1 shrink-0">`;
                    board.forEach((cell, idx) => {
                        const col = idx % 7;
                        const canClick = isMyTurn && (board[col] === '');
                        let cellContent = `<div class="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-slate-100 dark:bg-slate-900 border border-blue-500/50 dark:border-blue-900 shadow-inner"></div>`;
                        if (cell === 'R') {
                            ...
                            cellContent = `<div class="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-emerald-300 to-green-500 shadow-md border-2 border-emerald-200 animate-scale-in"></div>`;
                        }
                        const clickHandler = canClick ? `onclick="window.makeConnect4ProMaxMove('${post.id}', ${col})"` : '';
                        const hoverClass = canClick ? 'cursor-pointer hover:opacity-85' : 'cursor-default';
                        gridHtml += `
                            <div ${clickHandler} class="w-full h-full flex items-center justify-center select-none ${hoverClass}">
                                ${cellContent}
                ...
                if (post.gameWinner === 'draw') {
                    ...
                    const winnerName = post.gameWinner ? (window.globalUsersCache[post.gameWinner]?.name || 'Someone') : 'Someone';
                    const winnerSymbol = post.gameWinner === playerR ? '🔴' : (post.gameWinner === playerY ? '🟡' : (post.gameWinner === playerB ? '🔵' : '🟢'));
                    outcomeHtml = `<div class="inline-flex items-center gap-2 bg-emerald-500/15 text-emerald-800 dark:text-emerald-300 border border-emerald-500/30 font-bold px-4 py-1.5 rounded-full text-xs text-center shadow-sm mt-2"><i class="fa-solid fa-trophy text-amber-400"></i> <span>${winnerSymbol} ${escapeHtml(winnerName)} won Connect 4 Pro Max!</span></div>`;
                }

                const winningLine = Array.isArray(post.proFourWinningLine) ? post.proFourWinningLine : [];
                let gridHtml = `<div class="grid grid-cols-7 grid-rows-9 gap-1 p-2 bg-blue-600/90 dark:bg-blue-900/90 rounded-2xl border-2 border-blue-700 dark:border-blue-950 w-64 h-80 mx-auto my-2 opacity-95 shrink-0">`;
                board.forEach((cell, idx) => {
                    const isWinCell = winningLine.includes(idx);
                    let cellContent = `<div class="w-6 h-6 rounded-full bg-slate-100/70 dark:bg-slate-900/70 border border-blue-400/50 dark:border-blue-950"></div>`;
                    if (cell === 'R') {
                        ...
                        cellContent = `<div class="w-6 h-6 rounded-full bg-gradient-to-br from-emerald-300 to-green-500 shadow border border-emerald-200 ${isWinCell ? 'ring-4 ring-yellow-300 scale-110' : ''}"></div>`;
                    }
                    gridHtml += `<div class="w-full h-full flex items-center justify-center select-none">${cellContent}</div>`;
                });
                gridHtml += `</div>`;
                ...
                gameHtml = `
                    <div class="mt-3 mb-2 p-4 bg-gray-50 dark:bg-slate-900/50 rounded-2xl border border-gray-200 dark:border-slate-700 flex flex-col items-center opacity-90 text-center">
                        ${prizeStr}
                        <h4 class="font-black text-gray-700 dark:text-gray-300 text-base mb-1">🟢🔴 Connect 4 Pro Max Ended</h4>
                        ${gridHtml}
                        ${outcomeHtml}
                    </div>`;
            }
